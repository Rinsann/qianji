...
export default class Statistics extends Vue {
  ...
  get chartOptions() {
    ...
    const keys = this.keyValuePairs.map(item => item.key);
    const values = this.keyValuePairs.map(item => item.value);
    return {
      grid: {
        left: 0,
        right: 0,
      ...
      series: [{
        symbol: 'circle',
        symbolSize: 6,
        itemStyle: {borderWidth: 10, color: 'dodgerblue', borderColor: 'dodgerblue'},
        data: values,
        type: 'line'
      }],
